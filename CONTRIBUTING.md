...
1. In [vss-extension.json](./vss-extension.json ) in the `publisher` property, replace `boexler` with your `publisherId` 
1. In [configs/dev.json](./configs/dev.json ) in the `id` property, replace `boexler` with your `publisherId` 
1. Run `npm install`
